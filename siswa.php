...
<!-- ---------------------- TAMBAH DATA ---------------------- -->
    <nav class="navbar">
      <button type="button" class="btn btn-primary my-2" data-toggle="modal" data-target="#Modal_input">
        <i class="fas fa-plus-square"> Tambah Data Siswa</i>
      </button>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button submit" value="tambah" name="POST" class="btn btn-primary" action="siswa_aksi.php?action=tambah">Ganti</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <table class="table table-striped table-bordered table-font">
    ...
    <?php include 'db_Connection.php'; ?>
      <?php 
        if(isset($_GET['cari'])){
          $cari = $_GET['cari'];
          $query = ("SELECT * FROM siswa WHERE nama_lengkap like '%".$cari."%'");  
          global $connect;
          $query_data=mysqli_query($connect, $query);
        } else{
          $query=("SELECT * FROM siswa ORDER BY no_induk ASC");
          global $connect;
          $query_data=mysqli_query($connect, $query);
        }
