...
<!-- ---------------------- JUDUL TABEL ---------------------- -->
    <h3><i class="far fa-envelope mt-2"></i> Red Card </h3><hr>
  ...
  <table class="table table-striped table-bordered table-font mt-2">
    ...
    <?php include 'db_Connection.php'; ?>
      <?php 
        if(isset($_GET['cari'])){
          $cari = $_GET['cari'];
          $query = ("SELECT * FROM redcard WHERE kepada like '%".$cari."%'");  
          global $connect;
          $query_data=mysqli_query($connect, $query);
        } else{
          $query=("SELECT * FROM redcard ORDER BY id_redcard ASC");
          global $connect;
          $query_data=mysqli_query($connect, $query);
        }
      ...
      <tr>
        <td align="center"><?php echo $row['id_redcard'];?></td>
        <td align="center"><?php echo $row['id_redcard'];?></td>
