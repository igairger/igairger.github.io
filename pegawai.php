...
<!-- ---------------------- JUDUL TABEL ---------------------- -->
    <h3><i class="fas fa-user-graduate mt-2"></i> Daftar Pegawai </h3><hr>

<!-- ---------------------- TAMBAH DATA ---------------------- -->
    <nav class="navbar">
      <button type="button" class="btn btn-primary my-2" data-toggle="modal" data-target="#Modal_input">
        <i class="fas fa-plus-square"> Tambah Data Pegawai</i>
      </button>

<!-- ---------------------- SEARCH DATA ---------------------- -->
    <form class="form-inline my-2 my-lg-0 ml-auto">
        <input class="form-control mr-sm-2" type="search" placeholder="Masukan nama pegawai" aria-label="Search" name="cari">
        <div class="icon">
          <button class="btn my-sm-0 text-white btn-info btn-block" type="submit">Cari</button>
        </div>
        <div class="icon">
          <a href="pegawai.php" class="btn fas fa-sync-alt my-sm-0 text-white btn-info btn-block bg-success ml-1 p-2"></a>
        </div>
    </form>
  ...
  <div class="modal fade" id="Modal_input" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="pegawai_aksi.php?action=tambah" method="tambah" enctype="multipart/form-data"> 
              <table class="table table-striped table-bordered table-font">
                  <tr>
                      <td><label for="id_pegawai">* ID pegawai</label></td>
                      <td> : <input id="input_txt" class="txt_title" type="text" name="id_pegawai" spellcheck="false" placeholder="cth: 201901001" required></td>
                  </tr>

                  <tr>
                      <td><label for="nama_pegawai">Nama pegawai</label></td>
                      <td> : <input id="input_txt" class="txt_title" type="text" name="nama_pegawai" spellcheck="false" placeholder="cth: Ujang Udin" required></td>
                  </tr>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button submit" value="tambah" name="POST" class="btn btn-primary" action="pegawai_aksi.php?action=tambah">Ganti</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <table class="table table-striped table-bordered table-font">
    ...
    <?php include 'db_Connection.php'; ?>
      <?php 
        if(isset($_GET['cari'])){
          $cari = $_GET['cari'];
          $query = ("SELECT * FROM pegawai WHERE nama_lengkap like '%".$cari."%'");  
          global $connect;
          $query_data=mysqli_query($connect, $query);
        } else{
          $query=("SELECT * FROM pegawai ORDER BY no_induk ASC");
          global $connect;
          $query_data=mysqli_query($connect, $query);
        }
      ...
      <tr>
        ...
        <td align="center">
          <i href="pegawai_aksi.php?action=delete&id=<?php echo $row['no_induk'];?>" 
          class="btn fas fa-trash-alt bg-danger p-2 text-white rounded"
          onclick="return confirm('Apakah Anda yakin akan menghapus data: <?php echo $row['nama_lengkap'];?>?');">
        </td>
      </tr>
