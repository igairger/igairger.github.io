...
<!-- ---------------------- JUDUL TABEL ---------------------- -->
    <h3><i class="far fa-envelope mt-2"></i> iMessage </h3><hr>
  ...
  <table class="table table-striped table-bordered table-font mt-2">
    ...
    <?php include 'db_Connection.php'; ?>
      <?php 
        if(isset($_GET['cari'])){
          $cari = $_GET['cari'];
          $query = ("SELECT * FROM imessage WHERE kepada like '%".$cari."%'");  
          global $connect;
          $query_data=mysqli_query($connect, $query);
        } else{
          $query=("SELECT * FROM imessage ORDER BY id_imessage ASC");
          global $connect;
          $query_data=mysqli_query($connect, $query);
        }
      ...
      <tr>
        <td align="center"><?php echo $row['id_imessage'];?></td>
        <td align="center"><?php echo $row['id_imessage'];?></td>
        <td align="center"><?php echo $row['kepada'];?></td>
        <td align="center"><?php echo $row['isi_imessage'];?></td>
